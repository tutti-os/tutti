...
changelog 都从同一个文件读取，避免同一版本拆成多个小文件后不好管理。
...
- 飞书卡片读取它，展示中文摘要。
- GitHub Release body 读取它，写入中英文摘要。
...
本次更新
- 功能优化：稳定包下载入口只指向正式 release，避免 RC 包被外部用户下载。
- Bug Fix：修复自动更新安装流程，减少 macOS 更新后未正确安装的情况。
- 开发者 / 发布：新增 release summary，为飞书、GitHub Release 和 changelog 提供统一来源。
...
7. 写出本次 workflow 内部使用的 `release-summary.json`，其中包含飞书展示内容、
   GitHub Release body 双语 markdown、以及 changelog entry。
...
4. 把双语摘要 upsert 到 GitHub Release body 的 summary marker。
...
- 触发 RC workflow：
  ...
  - 飞书卡片展示中文 RC 摘要。
  - GitHub pre-release body 展示中英文摘要。
...
- GitHub Release body 是否需要中英双语，还是只保留中文？
