...
> Historical evaluation only. The Luna subagent experiment documented here
> has been superseded by provider-neutral, Session-scoped RTK saver mode and is
> not the current runtime contract. The current mode supports every Agent
> provider and does not install globally. See
