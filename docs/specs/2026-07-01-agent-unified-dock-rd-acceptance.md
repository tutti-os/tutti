...
2. 新增 `AgentDockLayout = "legacySplit" | "unified"` presentation preference，
   默认 `unified`。
3. 新增 daemon-owned `agent_targets` registry，第一阶段只初始化 Codex、Claude Code
   与 Tutti Agent system targets。
...
9. AgentGUI `@` 面板把 agent 候选从 `workspace-app` 迁出到独立
   ...
   `agent-claude-code` / `agent-tutti-agent` 伪应用。
...
- 不保留历史 `mention://workspace-app/agent-codex?...` /
  ...
  `mention://workspace-app/agent-tutti-agent?...` 的特殊打开或解析兼容；它们可作为
  历史展示 token 存在，但不再作为新 `@` 查询或 launch authority。
- 不删除 `tutti codex start` / `tutti claude start` /
  `tutti tutti-agent start` 等 provider 便利 CLI 命令；breaking change 只作用在
  `@` mention discovery identity。
...
- 第一阶段 Agents tab 展示 `local:codex`、`local:claude-code` 与
  `local:tutti-agent`。
...
- 历史 `mention://workspace-app/agent-codex?...` /
  `mention://workspace-app/agent-claude-code?...` /
  `mention://workspace-app/agent-tutti-agent?...` 不再需要可打开；新查询不得继续生成这种
  mention。
...
- `local:tutti-agent`: provider `tutti-agent`，launch ref `{ "type": "local_cli", "provider": "tutti-agent" }`。
...
| Agent 从 `workspace-app` 迁到 `agent-target`   | RISK | 当前 external `@` provider id 列表没有 `agent-target`；rich-text service 对 `agent-codex` / `agent-claude-code` / `agent-tutti-agent` 有特判。 | 新增 public provider id；默认查询包含 `agent-target`；显式 `workspace-app` 查询不再返回 agents。                        |
...
3. Conversation filter 纯模型已落地
   ...
   - `all` 默认只覆盖本 PRD 第一阶段目标 provider: Codex、Claude Code 与
     Tutti Agent。
...
- Fresh workspace 自动存在 `local:codex`、`local:claude-code` 与
  `local:tutti-agent` system targets。
...
- 选择 Codex / Claude Code / Tutti Agent 插入
  `mention://agent-target/<targetId>`。
- 新 `@` 查询不再生成 `mention://workspace-app/agent-codex?...` /
  `mention://workspace-app/agent-claude-code?...` /
  `mention://workspace-app/agent-tutti-agent?...`。
...
- `DesktopRichTextAtService`: `agent-target` query maps Agent Target rows; `workspace-app`
  excludes `agent-codex` / `agent-claude-code` / `agent-tutti-agent`。
...
7. 在 AgentGUI 输入 `@`: Codex、Claude Code 与 Tutti Agent 出现在 Agents，
   而不是 Apps。
