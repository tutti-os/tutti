...
- Mobile 和 Desktop 各自生成持久设备密钥；私钥只保存在设备安全存储中。
...
pairing lifecycle 和 paired-device rendezvous 已完成；Tutti Personal Desktop 与
Android 客户端尚未接入这些端点，因此 M2 还未达到端到端完成条件。
