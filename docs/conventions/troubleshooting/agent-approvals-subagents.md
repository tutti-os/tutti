...
- Fix: merge partial tool updates into the per-turn snapshot, expose the saved
  ...
  is for editing files. A lone `grantRoot` outside a file-change approval
  remains a labeled path detail. These remain approval previews, not evidence
  that a file change executed.
