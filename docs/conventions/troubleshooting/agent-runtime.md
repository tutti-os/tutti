...
- [AgentGUI keeps the old model after a model-plan switch](./agent-provider-setup.md#agentgui-keeps-the-old-model-after-a-model-plan-switch)
...
- [Codex model-plan session fails because wire_api chat is unsupported](./agent-provider-setup.md#codex-model-plan-session-fails-because-wireapi-chat-is-unsupported)
- [OpenRouter model-plan turn fails because namespace tools are unsupported](./agent-provider-setup.md#openrouter-model-plan-turn-fails-because-namespace-tools-are-unsupported)
