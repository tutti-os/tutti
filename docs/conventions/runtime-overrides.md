...
| `TUTTI_CLAUDE_CODE_BINARY_BASE_URL`     | [Desktop Release](./desktop-release.md)         | Overrides the CDN base URL used to provision the pinned Claude native binary.                    |
| `TUTTI_CLAUDE_CODE_FALLBACK_EXECUTABLE` | This document                                   | Internal handoff for a managed or PATH Claude executable when the SDK has no bundled binary.     |
