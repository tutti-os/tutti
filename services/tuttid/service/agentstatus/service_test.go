...
func TestDefaultRegistryUsesTuttiAgentManagedNPMInstaller(t *testing.T) {
	...
	if install.ManagedNPM.PackageVersion != tuttiAgentRequiredVersion {
		t.Fatalf("PackageVersion = %q, want %q", install.ManagedNPM.PackageVersion, tuttiAgentRequiredVersion)
	}
	...
	if specs[0].AdapterPackage.Name != "@tutti-os/tutti-agent" || specs[0].AdapterPackage.Version != tuttiAgentRequiredVersion {
		t.Fatalf("AdapterPackage = %+v, want @tutti-os/tutti-agent@%s", specs[0].AdapterPackage, tuttiAgentRequiredVersion)
	}
}
...
func TestTuttiAgentAdapterVersionMismatchRequiresInstall(t *testing.T) {
	...
	if status.Adapter.RequiredVersion != tuttiAgentRequiredVersion {
		t.Fatalf("Adapter.RequiredVersion = %q, want %q", status.Adapter.RequiredVersion, tuttiAgentRequiredVersion)
	}
	action := firstAction(t, status.Actions)
	if action.ID != ActionInstall {
		t.Fatalf("first action ID = %q, want %q", action.ID, ActionInstall)
