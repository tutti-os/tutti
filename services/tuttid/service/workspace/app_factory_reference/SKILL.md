...
- To give an app local agent run access to Tutti's dynamic CLI skills, prefer the `@tutti-os/agent-acp-kit/tutti` helper instead of hand-writing `$TUTTI_CLI agent tutti-cli-skill-bundle` execution and response parsing in each app.
...
Agent app main flows should expose provider choices from Tutti workspace-app scoped `preferences/agent`, `agent-providers/status`, and `agent-providers/{provider}/composer-options` APIs rather than app-local provider allowlists or kit runtime provider lists. Keep unavailable providers visible as disabled with a reason, use the `@tutti-os/agent-acp-kit/tutti` catalog helpers when available, and keep only a thin whole-catalog failure fallback that shows Codex and Claude with `default` models.
