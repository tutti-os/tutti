...
Use the bundled Tutti CLI for workspace context. The guide below lists built-in commands plus a compact index of workspace apps; to get a specific app's commands, run `{{CLI_COMMAND}} app commands --app-id <app id> --json` (or `--all`) on demand.
