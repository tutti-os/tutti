...
// ReferenceResolver reports the consumers currently referencing a plan.
// Implementations aggregate agent targets, model usage policies, and
// workspace apps.
...
func (s *Service) PlanReferences(ctx context.Context, workspaceID string, planID string) ([]modelplanbiz.Reference, error) {
	...
	references, err := s.References.ListModelPlanReferences(ctx, strings.TrimSpace(workspaceID), strings.TrimSpace(planID))
	if err != nil {
		return nil, err
	}
	if references == nil {
