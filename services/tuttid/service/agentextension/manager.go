...
func (m *Manager) reconcileSource(ctx context.Context, source tuttitypes.AgentExtensionSource) (Installation, error) {
	...
	preferManagedRuntime := false
	if installed, err := m.loadActive(source.Key); err == nil {
		preferManagedRuntime = installed.PreferManagedRuntime
		...
		if m.releaseRequiresUserRuntimeUpdate(ctx, installed, record.Release) {
			return installed, nil
		}
	}
	artifact, err := m.downloadReleaseArtifact(ctx, record.Release)
	if err != nil {
		return Installation{}, err
	}
	return m.installVerifiedRelease(record.Release, artifact, source, preferManagedRuntime)
}
