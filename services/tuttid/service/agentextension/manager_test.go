...
func testPackageZIPFor(t *testing.T, manifest Manifest, discovery string) []byte {
	...
	if manifest.Profiles.AccountUsage != "" {
		files[manifest.Profiles.AccountUsage] = []byte(`{"schemaVersion":"tutti.agent.account-usage-probe.v1","runtime":{"package":"@example/gemini-account-usage@1.0.0","executable":"${installRoot}/node_modules/.bin/gemini-account-usage","args":["--output","json"],"timeoutMs":10000}}`)
	}
