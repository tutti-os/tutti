...
func (r *recordingModelCatalog) ListModels(context.Context, string) (AgentModelCatalogResult, error) {
	r.calls++
...
func (r *recordingFirstUse) MarkFirstUseFailure(_ context.Context, workspaceID string, planID string, agentTargetID string, agentSessionID string, model string) error {
	r.failureCalls = append(r.failureCalls, workspaceID+"/"+planID+"/"+agentTargetID+"/"+agentSessionID+"/"+model)
	return nil
}
...
func TestResolveModelPlanEndpointMatchesProviderProtocol(t *testing.T) {
	...
	// Cursor/OpenCode only reserve the contract in this iteration.
...
func TestObserveAgentSessionStateMarksFirstUseOnCompletedTurn(t *testing.T) {
	...
	if len(firstUse.failureCalls) != 1 || firstUse.failureCalls[0] != "ws/mp-1/local:codex/session-1/plan-default" {
		t.Fatalf("first-use failure calls = %v", firstUse.failureCalls)
	}
...
func TestObserveAgentSessionStateRebuildsFirstUseFromPersistedSnapshot(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	service, firstUse := newPlanBoundService(modelplanbiz.ProtocolOpenAI, true)
	resolution := service.resolveModelPlan(ctx, "ws", "local:codex", "codex", "plan-default")
	model := "plan-default"
	...
	service.ObserveAgentSessionState(ctx, agentsessionstore.ReportSessionStateInput{
		WorkspaceID:    "ws",
		AgentSessionID: "session-restarted",
		State: agentsessionstore.WorkspaceAgentSessionStateUpdate{
			TurnLifecycle: &agentsessionstore.WorkspaceAgentTurnLifecycle{Phase: "settled", Outcome: &completed},
		},
	}, agentsessionstore.ReportSessionStateReply{})
	if len(firstUse.calls) != 1 || firstUse.calls[0] != "ws/mp-1/local:codex/session-restarted/plan-default" {
		t.Fatalf("first use calls = %v", firstUse.calls)
	}
}
