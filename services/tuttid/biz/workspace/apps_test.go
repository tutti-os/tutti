...
func TestParseAppManifestJSONAcceptsReferencesSearchEndpoint(t *testing.T) {
	t.Parallel()
	...
	if _, _, err := ParseAppManifestJSON([]byte(
		...
		t.Fatalf("ParseAppManifestJSON() error = %v, want nil for references.searchEndpoint", err)
	}
}
