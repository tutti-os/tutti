...
func (s *SQLiteStore) GetWorkspaceWorkflowSnapshot(ctx context.Context, workspaceID string, workflowID string) (workflowbiz.Snapshot, error) {
	...
	workflow, err := getWorkspaceWorkflow(ctx, s.db, workspaceID, workflowID)
	if err != nil {
	...
	if snapshot.Revisions, err = listWorkflowPlanRevisions(ctx, s.db, workspaceID, workflowID); err != nil {
		return workflowbiz.Snapshot{}, err
	}
	if snapshot.Checkpoints, err = listWorkflowCheckpoints(ctx, s.db, workspaceID, workflowID); err != nil {
		return workflowbiz.Snapshot{}, err
	}
	if snapshot.TurnLinks, err = listWorkflowTurnLinks(ctx, s.db, workspaceID, workflowID); err != nil {
		return workflowbiz.Snapshot{}, err
	}
	if snapshot.Operations, err = listWorkflowOperations(ctx, s.db, workspaceID, workflowID); err != nil {
		return workflowbiz.Snapshot{}, err
	}
	return snapshot, nil
}
...
func (s *SQLiteStore) AppendWorkspaceWorkflowTurnLink(ctx context.Context, workspaceID string, link workflowbiz.WorkflowTurnLink) error {
	...
	return insertWorkflowTurnLink(ctx, s.db, workspaceID, normalized)
}
...
func (s *SQLiteStore) RecordWorkspaceWorkflowOperation(ctx context.Context, workspaceID string, operation workflowbiz.WorkflowOperation) error {
	...
	return insertWorkflowOperation(ctx, s.db, workspaceID, normalized)
}
...
`
	...
	operation, err := getWorkflowOperation(ctx, s.db, input.WorkspaceID, input.WorkflowID, input.OperationID)
	if err != nil {
