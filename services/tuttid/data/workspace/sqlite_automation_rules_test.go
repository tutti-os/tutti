...
func TestAutomationRulesMigrationBackfillsBoundReviewPolicy(t *testing.T) {
	...
	if _, err := store.db.ExecContext(ctx, `DELETE FROM tuttid_schema_migrations WHERE id IN (?, ?, ?, ?, ?)`, schemaMigrationWorkspaceAgentsV1, schemaMigrationWorkspaceAgentsV2, schemaMigrationWorkspaceAgentsV3, schemaMigrationWorkspaceAgentsV4, schemaMigrationAutomationRulesV1); err != nil {
		t.Fatalf("reset migration markers error = %v", err)
	}
	if _, err := store.db.ExecContext(ctx, `DROP TABLE workspace_agents; DROP TABLE automation_rule_session_overrides; DROP TABLE automation_rules;`); err != nil {
