...
func (s *SQLiteStore) applyWorkspaceIssuesV7(ctx context.Context) error {
	...
	if !hasColumn {
		if _, err := s.writeDB.ExecContext(ctx, "ALTER TABLE workspace_issues ADD COLUMN sequential_execution INTEGER NOT NULL DEFAULT 0;"); err != nil {
...
func (s *SQLiteStore) applyWorkspaceIssuesV8(ctx context.Context) error {
	...
	if !hasColumn {
		if _, err := s.writeDB.ExecContext(ctx, "ALTER TABLE workspace_issues ADD COLUMN parallel_execution INTEGER NOT NULL DEFAULT 0;"); err != nil {
...
func (s *SQLiteStore) applyWorkspaceIssuesV9(ctx context.Context) error {
	...
	if !hasColumn {
		if _, err := s.writeDB.ExecContext(ctx, "ALTER TABLE workspace_issues ADD COLUMN dispatch_paused INTEGER NOT NULL DEFAULT 0;"); err != nil {
...
// applyWorkspaceIssuesV14 records the per-task acceptance bypass from the
