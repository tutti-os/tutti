...
// the durable goal reconcile inbox, event deliveries, unrecoverable stale
// turns, and finally the adapter-specific worktree-isolation sweep.
func (h *Host) Recover(ctx context.Context) error {
	...
	if err := h.RecoverEventDeliveries(ctx); err != nil {
		return err
	}
