...
3. Agent launching is not a workspace-app workflow. New agent references use `mention://agent-target/...` and `$tutti-handoff`; never translate an app id into a provider-specific agent command.
   {{if hasFamily "issue"}}4. If `appId` is `issue-manager` and the user asks issue/task work, read and follow injected `$issue-manager` for issue/task context and workflows before using generic workspace app command matching.
   {{end}}
4. For any workspace app mention, if the injected command guide exposes app-specific CLI capabilities for the requested operation, use those persisted app commands instead of provider-native or OS-native substitutes. The external app owns the exact command syntax, validation rules, and domain-specific restrictions through its capability metadata, command guide, and command handler errors.
5. App commands inherit the caller agent session working directory unless their advertised schema provides and the caller supplies another working directory.
6. For other app ids, read `command-guide.md`, find command entries whose metadata says `App id: <appId>`, then run the exact backticked command path shown there. The actual CLI prefix is `{{.CLICommand}}` and may be a production command or a local debug command. If no entry is listed, refresh the command guide or skill bundle capability reference that preserves `App id:` metadata before deciding the app command is unavailable.
7. If several apps have similar names, match by `appId` from the mention, not only by the visible label.
8. Use the listed `{{.CLICommand}} <scope> <command>` examples to inspect or invoke the app. Do not derive a command path from the skill slug.
9. Prefer JSON when the command entry advertises JSON support and output is used as context for reasoning.
