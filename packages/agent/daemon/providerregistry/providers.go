...
func tuttiAgentDescriptor() ProviderDescriptor {
	return ProviderDescriptor{
		...
		Runtime:  RuntimeDescriptor{Kind: RuntimeKindCodexAppServer, Name: "tutti-agent-app-server", Command: []string{"tutti-agent", "app-server"}, ClientInfoName: "tutti_agent", AuthRequiredMessage: "Tutti Agent requires authentication. Sign in to Tutti on this device (or run `tutti-agent login`), then retry this session.", AuthRefreshOwner: RuntimeAuthRefreshOwnerRuntime, Endpoint: RuntimeEndpointDescriptor{ModelPlanProtocol: ModelPlanProtocolOpenAI}},
		Status: StatusDescriptor{
