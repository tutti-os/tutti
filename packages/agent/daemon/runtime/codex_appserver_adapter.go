...
func NewCodexAppServerAdapterWithHostMetadata(transport ProcessTransport, host HostMetadata) *CodexAppServerAdapter {
	return newCodexAppServerAdapterWithHostMetadataAndResolver(transport, host, nil)
}
...
func codexClientInfoParams(host HostMetadata, command []string, env []string) map[string]any {
	return codexClientInfoParamsForVersion(host, resolveCodexCLIVersion(command, env))
}
