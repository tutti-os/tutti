...
func (a *ClaudeCodeSDKAdapter) sidecarTurnEvents(adapterSession *claudeSDKAdapterSession, session Session, turnID string, event claudeSDKSidecarEvent) ([]activityshared.Event, bool, error) {
	...
	case "usage_updated":
		...
		if !goalClearControlTurn {
			if inputTokens, outputTokens, ok := a.applyClaudeSDKTurnTokenUsage(adapterSession, rootTurnID, event.Payload); ok {
				if flush, ok := turnTokenUsageEvent(session, rootTurnID, inputTokens, outputTokens); ok {
					events = append(events, flush)
				}
			}
		}
		if adapterSession.applyUsageUpdated(event.Payload) {
			if usageEvent, ok := normalizedUsageUpdatedEvent(session); ok {
