...
func (*Store) recordTurnTransitionTx(
	...
	tokenUsageJSON, err := marshalNullableTurnTokenUsage(merged.TokenUsage)
	if err != nil {
		return Turn{}, false, fmt.Errorf("encode workspace agent turn token usage: %w", err)
	}
...
func mergeTurnTransition(existing Turn, hasExisting bool, transition TurnTransition, phase string, occurred int64, now int64) Turn {
	...
	if transition.TokenUsage != nil {
		tokenUsage := *transition.TokenUsage
		merged.TokenUsage = &tokenUsage
	}
...
// marshalNullableTurnTokenUsage mirrors marshalNullableJSONMap for the typed
// token counter pair: nil stays SQL NULL so providers without an input/output
// split never persist a fabricated zero counter.
func marshalNullableTurnTokenUsage(value *TurnTokenUsage) (any, error) {
	if value == nil {
		return nil, nil
	}
...
func unmarshalTurnTokenUsage(input string) (TurnTokenUsage, error) {
	var usage TurnTokenUsage
	if err := json.Unmarshal([]byte(input), &usage); err != nil {
		return TurnTokenUsage{}, err
	}
	return usage, nil
}
