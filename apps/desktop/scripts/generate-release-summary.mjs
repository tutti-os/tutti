...
function sanitizeSummary(candidate) {
  ...
  const normalizeLocale = (locale, fallbackLocale) => ({
    headline:
  ...
  return {
    ...
    zh: normalizeLocale(candidate?.zh, fallback.zh),
    en: normalizeLocale(candidate?.en, fallback.en)
  };
}
...
async function buildAgnesSummary(input, apiKey) {
  const prompt = [
    "You summarize desktop app releases for humans.",
