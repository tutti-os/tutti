      ...
      return [
        createRichTextTriggerProvider<WorkspaceModelAtItem>({
          ...
          async query(searchInput) {
            ...
            const response = await tuttidClient.listWorkspaceModelPlans(
              input.workspaceId
            );
            if (searchInput.abortSignal?.aborted) {
              return [];
          ...
          async resolveMention(identity) {
            ...
            return resolveMentionSafely(async () => {
              const response =
                await tuttidClient.listWorkspaceModelPlans(workspaceId);
...
function workspaceModelAtItems(input: {
  keyword: string;
  maxResults?: number;
  plans: Awaited<ReturnType<TuttidClient["listWorkspaceModelPlans"]>>["plans"];
  workspaceId: string;
