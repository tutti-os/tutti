...
test("desktop agent GUI workbench host input tracks privacy-safe engagement events", async () => {
  ...
  const analytics =
    hostInput.createAgentGUIEngagementAnalytics("standalone_agent");
  const context = {
    agentSessionId: "session-1",
  ...
  await analytics.onChatPanelExposed?.(context);
  await analytics.onChatInputFocused?.({
    ...context,
    focusMethod: "keyboard"
  });
  await analytics.onChatInputContentEntered?.({
    ...context,
